...
int32_t main(int32_t argc, char **argv) {
	...
	// create window
	String title = String::format("%s Tellusim::VKSpirv", window.getPlatformName());
	...
	if(!pipeline.loadShaderSPIRV(Shader::TypeVertex, "main_shader_v.spv")) return 1;
	if(!pipeline.loadShaderSPIRV(Shader::TypeFragment, "main_shader_f.spv")) return 1;
	...
	// create texture
	Texture texture = device.loadTexture("model.png");
	if(!texture) return 1;
	
	// load mesh
	Mesh mesh;
	if(!mesh.load("model.usda")) return 1;
	
	// create normals
	if(!mesh.createNormals()) return 1;
