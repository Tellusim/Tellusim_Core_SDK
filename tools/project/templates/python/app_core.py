...
def main(argv):
	...
	def create():
		...
		# create pipeline
		if RENDER_TRIANGLE and not create_pipeline():
			return False
	...
	def render(command):
		...
		if RENDER_TRIANGLE:
			...
			angle = Time.seconds() * 64.0
			iaspect = float(window.getHeight()) / window.getWidth()
			command.setUniform(0, Matrix4x4f.rotateZ(angle) * Matrix4x4f.scale(iaspect, 1.0, 1.0))
